                        ...
                        <tr>
                            ...
                            <div class="space-y-2">
                                    <div class="flex flex-col space-y-2">
                                        ...
                                        @php
                                        $starTime = $hourBlock->copy()->addMinutes($i* $apointment_duration);
                                        $endTime= $starTime->copy()->addMinutes($apointment_duration);
                                        @endphp


                                        <label>
                                            <input type="checkbox" 
                                            x-model="schedule['{{$indexDay}}']['{{$starTime->format('H:i:s')}}']"
                                            class="h-4 w-4 text-blue-600 border-gray-300  rounded focus:ring-blue-500">
                                            <span class="ml-2 text-sm font-medium text-gray-700">
                                            {{$starTime->format('H:i')}} - {{$endTime->format('H:i')}}
                                            </span>
                                        </label>
                                        @endfor
                                    </div>
