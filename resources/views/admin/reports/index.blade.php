        ...
        <div class="bg-white p-4 rounded-lg shadow-md border border-gray-200 mb-6">
            ...
            <div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <!-- Selector de otro usuario (opcional) -->
                    <div class="flex-1">
            ...
            <div class="mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <!-- Selectores dinámicos según el tipo de periodo -->
                    ...
                    <!-- Selector de semana (oculto por defecto) -->
                    ...
                    <!-- Selector de día (oculto por defecto) -->
            ...
            <div class="flex justify-between items-center pt-4 mt-4 border-t border-gray-200">
                ...
                <div class="text-sm text-gray-600">
                    @if(request()->anyFilled(['filter_month', 'filter_year', 'filter_week', 'filter_day', 'start_date']))
                        ...
                        @elseif(request('filter_week'))
                            Semana {{ substr(request('filter_week'), 6) }} de {{ substr(request('filter_week'), 0, 4) }}
                        @elseif(request('filter_day'))
                            {{ \Carbon\Carbon::parse(request('filter_day'))->isoFormat('DD MMMM YYYY') }}
                        @elseif(request('start_date') && request('end_date'))
                            {{ \Carbon\Carbon::parse(request('start_date'))->isoFormat('DD/MM') }} al {{ \Carbon\Carbon::parse(request('end_date'))->isoFormat('DD/MM/YYYY') }}
                        @endif
                    @else
        ...
        <div class="relative overflow-x-auto bg-white shadow-sm rounded-lg border border-gray-200">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200">
                ...
                <div class="relative w-64">
                    ...
                    <input type="text" name="search" placeholder="Buscar en resultados..." 
            ...
            <table class="w-full text-sm text-left text-gray-700">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-t border-b border-gray-300">
                    <tr>
                            ...
                            @if($consulta->appointment)
                                <div class="space-y-1">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="text-sm">
                                            @if($consulta->appointment->start_time)
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="text-sm">
                                            @if($consulta->appointment->end_time)
                                                {{ \Carbon\Carbon::parse($consulta->appointment->end_time)->format('H:i') }}
                                            @else
                                                N/A
                                            @endif
                                        </span>
...
<!-- JavaScript para mostrar/ocultar selectores dinámicos -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePeriodSelectors() {
        ...
        Object.values(selectors).forEach(selector => {
            if (selector) {
                ...
                // Resetear valores si no es el seleccionado
                if (selector !== selectors.month && selector !== selectors.year) {
                    const inputs = selector.querySelectorAll('input, select');
                    inputs.forEach(input => {
                        if (input.name !== selectedType) {
                            input.value = '';
                        }
                    });
                }
    ...
    // Auto-seleccionar mes/año actual si no hay filtros
    if (!{{ request()->anyFilled(['filter_month', 'filter_year', 'filter_week', 'filter_day', 'start_date']) ? 'true' : 'false' }}) {
        const monthSelect = document.querySelector('select[name="filter_month"]');
        const yearSelect = document.querySelector('select[name="filter_year"]');
